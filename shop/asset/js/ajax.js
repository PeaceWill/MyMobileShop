// Link api để gọi tới 
var link_client_api = 'https://88c772570a5b.ngrok.io/shop/view/client/client-api.php';
var link_admin_api = 'https://88c772570a5b.ngrok.io/shop/view/admin/admin-api.php';
...
function renderHotProduct(productObj) {
    document.querySelector('#card-content-value').innerText = productObj['hot']['number']['number'];
    var htmlHotProduct = '',
        htmlNotHotProduct = '';
    htmlHotProduct = productObj['hot']['product'].map((product, index) => {
        return `
            <ul class="product-record hover-08">
                <li class="product-info width-42px" style="color: 9d9b9e;">${index + 1}</li>
                ...
                <li class="product-info width-20">
                    <a class=del-action href="admin-api.php?action=delete&id=${product.id}" onclick="return pushProductToHot(${product.id})">
    ...
    htmlNotHotProduct = productObj['not-hot'].map((product, index) => {
        return `
            <ul class="product-record hover-08">
                ...
                <li class="product-info width-20">
                    <a class=add-action href="admin-api.php?action=add&id=${product.id}" onclick="return dropHotProduct(${product.id})">
    ...
    document.querySelector('#hot__product__content').innerHTML = htmlHotProduct.join('');
    document.querySelector('#not-hot__product__content').innerHTML = htmlNotHotProduct.join('');
}
                        ...
                        <a href="product-detail.php?id=${item.id}" class="product__item ">
                            ...
                            <div class="product__item-body">
                                ...
                                <div class="product__item-content">
                                    <span class="product__item-price">${Number(item.price).toLocaleString('us')}đ</span>
                                </div>
                            </div>
                        </a>
        ...
        $.ajax({
            ...
            success: (data) => {
                if (data.includes('message')) {
                    ...
                    document.querySelector('#modal__error__message-signup').innerHTML = 'Tài khoản đã tồn tại';
                }
            },
        });
    }
...
function updateUserDetail() {
    ...
    form.onsubmit = (event) => {
        ...
        $.ajax({
            ...
            success: (data) => {
                data = JSON.parse(data);
                renderResponseUserProfile(data, '#profile__form__update-detail--response');
            }
        });
    }
}
...
function updateUserPassword() {
    ...
    form.onsubmit = (event) => {
        ...
        $.ajax({
            ...
            success: (data) => {
                data = JSON.parse(data);
                renderResponseUserProfile(data, '#profile__form__update-password--response');
...
function addToCart() {
    ...
    if (addButton.length > 1) {
        addButton.forEach((item) => {
            item.onclick = () => {
                let url = (item.parentElement.parentElement.querySelector('a').href).split('?id=');
                $.ajax({
                    url: link_client_api,
                    type: 'POST',
        ...
        if (addButtonWithNumber.onclick = () => {
                    ...
                    <li class="cart__nav__list-item" id="cart__item-${data.id}">
                        <a class="cart__nav__list-link" href="">
                            ...
                            <div class="cart__nav__list-item--info">
                                <span class="cart__item-price">${Number(data.price).toLocaleString('us')}</span>
                                <span class="cart__item-number">x${data.number}</span>
                            </div>
                        </a>                             
                    </li>`;
    ...
    if (data.length > 0) {
        var html = data.map((item) => {
            return `<li class="search__result-item">
                        <a href="product-detail.php?id=${item.id}" class="search__result-link">
                            <div class="row">
        ...
        item.onclick = () => {
            $.ajax({
                ...
                success: (data) => {
                    ...
                    if (data.status == 1) {
                        $alert(data.message, '#06c1d4');
                        document.querySelector(`#feedback__customer-product-${Number(item.id)} textarea[name=message]`).value = '';
                    } else {
